...
async function eventhub_connect(eventhub, channel, peerName) {
    ...
    try {
        ...
        logger.info(check2)
        logger.info('check2.blockNumber:'+check2.blockNumber)
    ...
    regid = eventhub.registerChaincodeEvent(process.env.CHAINCODE_NAME, process.env.EVENT_NAME,
        async (events) => {
            ...
            for (const {chaincode_event, block_num, tx_id, tx_status} of events) {
                ...
                //1.파일 읽어옴
                //2.현재 이벤트로 받은 블럭넘버를 가지고 파일에서 그 블럭넘버 찾음. 
                //3.해당 블럭에 대해 트랜잭션이 있는지 검사해서 있으면 리턴. 
                //4.없으면 프로세스따라 쭉가서 체크포인트 저장. 
              
  
                const check = await checkpoint.load();
                let key = Object.keys(check);
// console.log(key)
                // console.log(check[key[0]].blockNumber)
                // console.log(check[key[0]].blockNumber)
                // console.log(check[key[0]].blockNumber)
                // console.log(check[key[0]].blockNumber)
                
                for( const bnum of key) {
                    // console.log(check[bnum].transactionIds.includes(tx_id))
                    if( check[bnum].transactionIds.includes(tx_id)) {
                        logger.info('##Listener## => _onEvent skipped transaction: %s', tx_id);
                        continue;
                    }
                    //트랜잭션이 빠진 블럭 넘버 
                    useCheckpoint = (Number(bnum) <= Number(block_num)); 
                    
                }


                // if(check[block_num].blockNumber == 'undefined') {
                //     console.log('언디파인드')
                // } else {
                    // useCheckpoint = Number(check[block_num].blockNumber || 0) <= Number(block_num);  
                    // if (check && check[block_num].transactionIds  && check[block_num].transactionIds.includes(tx_id)) {
                    //     logger.info('##Listener## => _onEvent skipped transaction: %s', tx_id);
                    //     continue;
                    // }
                // }
                ...
                if (tx_status != 'VALID') {
                    ...
                    if(useCheckpoint) await checkpoint.save(tx_status, block_num, events.length);   
                    continue;
                }
